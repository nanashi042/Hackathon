...
export function ChibiAvatar({ mood = 'neutral', size = 'md', className = '' }: ChibiAvatarProps) {
  ...
  const getEyeStyle = () => {
    if (blinkState) return { height: '2px' };
    
    switch (mood) {
      case 'sad':
        return { transform: 'scaleY(0.6)' };
      case 'happy':
        return { transform: 'scaleY(1.2)' };
      case 'thinking':
        return { transform: 'translateX(-1px)' };
      default:
        return {};
    }
  };
  ...
  const getMouthPath = () => {
    switch (mood) {
      case 'sad':
        return 'M8 12 Q12 10 16 12';
      case 'happy':
        return 'M8 10 Q12 14 16 10';
      case 'thinking':
        return 'M10 11 Q12 12 14 11';
      case 'speaking':
        return 'M10 10 Q12 13 14 10';
      default:
        return 'M10 11 L14 11';
    }
  };
  ...
  return (
    ...
    >
      {/* Glow effect */}
      <div className="absolute inset-0 rounded-full bg-gradient-to-br from-sakura/20 to-indigo/20 blur-md animate-pulse" />
      ...
      <div className="relative w-full h-full">
        <svg viewBox="0 0 24 24" className="w-full h-full">
          ...
          {!blinkState && (
            <>
              <circle cx="9.5" cy="8.5" r="0.5" fill="white" opacity="0.8" />
              <circle cx="15.5" cy="8.5" r="0.5" fill="white" opacity="0.8" />
            </>
          )}
      ...
      {/* Subtle shadow */}
      <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-2 w-8 h-2 bg-indigo/10 rounded-full blur-sm" />
    </div>
  );
}